...
> **Resume (bidirectional):** new context window → type **"start coding"** — the workspace `CLAUDE.local.md` operating anchor (§ START CODING — habitat-graph V3) resumes here. **G-ABI + core migration DONE** (`main@08d05a0`; PA-1 prep sealed in `EVIDENCE.md`); next = PA-1 grammar fan-out, first step golden-corpus sourcing (C-G1).
