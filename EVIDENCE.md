...
- `--update` incremental | [VBE] | `commands::update` — owner-only raw extraction state (Git-context/output scoped, with a legacy non-Git `.habitat-graph-state.json` path): blake3 content-hash diff → re-extract only changed/added files → `prune_graph` drops stale-file nodes + dangling edges → `build::merge` → re-analyze. **Honest C-4:** Leiden is re-run GLOBALLY when inputs change (the state saves AST parsing, not analysis); an unchanged run still refreshes public artifacts and hardens private state. Schema mismatch forces a full rebuild instead of a silent cross-taxonomy merge.
...
- merge driver | [VBE] | `build::merge_driver::merge3(base, ours, theirs)` — deterministic 3-way merge of `graph.json`: a node/edge/community present on either side is kept unless `base` shows it was deliberately deleted on the other side. Clean nodes use label identity; lossy public nodes/relations retain stable IDs plus base/branch provenance. The sorted result is byte-identical (R4), so git can auto-merge two branches' graphs with **no textual conflict**. Risk register (low): delete-vs-modify resolves silently to delete (intentional for a regenerable artifact).
...
- content IDs | [VBE] | `core::content_id(label) = blake3(label)[..4] as u32 (LE)`. `assemble` and clean-node merge identities assign `NodeId = content_id(label)` with deterministic collision probing; already projected public nodes retain their published IDs plus provenance. Effect: adding/removing one symbol no longer renumbers the others — minimal `graph.json` diffs (R4) + a stable 3-way merge. Preserving a raw label's public content ID also permits offline guesses for low-entropy labels; export redaction is not anonymization.
...
- wiring | [VBE] | `cli merge-driver %O %A %B` + `cli install-merge-driver --repo` (new subcommands in `commands::merge_driver`). `run_merge_driver` reads base/ours/theirs public `graph.json` → provenance-aware `build::merge3` → writes the canonical redacted projection back to `ours` (git's `%A`); `install` appends `graph.json merge=habitat-graph` to `.gitattributes` (idempotent, non-clobbering) + prints the `git config` lines.
...
- internal graph-state roundtrip | [VBE] | `schema::tests::json_roundtrip_preserves_graph` + deterministic `sorted()` (R4) + `two_graphs_same_content_serialize_identically`; public node-link projection is covered at D4.
...
- `habitat-graph-build` (assemble/dedup/merge) | [VBE] | `assemble` interns raw labels to content IDs; `dedup` keys exact ids/edges; `merge` re-interns clean labels while preserving lossy projected IDs/provenance, remaps edges and community members, and keeps manifest precedence deterministic. The original D3 seal was 347 workspace tests (build 78); the later public-redaction hardening is recorded above.
...
- `habitat-graph-export` (node-link json + report + obsidian) | [VBE] | `to_node_link` = graphify-compatible NetworkX node-link envelope (nodes/links, `Lnn` locations, weight 1.0/0.8, inline community) with shared deterministic redaction followed by render escaping; `render_report` and `render_vault` use the same boundary. Original D4 seal: 475 workspace tests (export 82); full cross-export hardening is recorded above.
